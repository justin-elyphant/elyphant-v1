-- Fix connection_established email trigger to use snake_case variable names
-- The template expects 'recipient_name' but trigger was passing 'recipientName'
...
BEGIN
  ...
  IF OLD.status IN ('pending', 'pending_invitation') AND NEW.status = 'accepted' THEN
    ...
    INSERT INTO email_queue (
      ...
      jsonb_build_object(
        'recipient_name', v_user1_profile.name,
    ...
    INSERT INTO email_queue (
      ...
      'connection_established',
      jsonb_build_object(
        'recipient_name', v_user2_profile.name,
