...
serve(async (req) => {
  ...
  try {
    ...
    // Always route via Zinc bizapi for Prime shipping, low latency, and high throughput.
    // Per Zinc bizapi docs, payment_method, retailer_credentials, and billing_address
    // MUST be omitted (they are not present in the payload below).
    console.log('⚡ BizAPI enabled — sending zma_flags: ["bizapi", "bizapi-only"]');

    const zincRequest: Record<string, any> = {
      addax: true, // CRITICAL: Enables ZMA processing
      zma_flags: ['bizapi', 'bizapi-only'], // CRITICAL: Routes order to Zinc bizapi
