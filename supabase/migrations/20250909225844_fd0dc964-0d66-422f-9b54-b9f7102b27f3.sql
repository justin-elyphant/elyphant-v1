...
-- Group gift contributions policies (corrected column name)
...
USING (EXISTS (
  ...
  AND ((select auth.uid()) = gp.coordinator_id OR 
       (select auth.uid()) = group_gift_contributions.contributor_id)
));
