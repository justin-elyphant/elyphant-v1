-- Update the handle_new_user function to properly handle Google OAuth data including username
...
BEGIN
  ...
  -- Handle username conflicts by adding suffix if needed
  WHILE EXISTS (SELECT 1 FROM public.profiles WHERE username = final_username) LOOP
    final_username := extracted_username || username_suffix::text;
    username_suffix := username_suffix + 1;
  END LOOP;
  
  -- Insert profile with proper data including username
