-- Add database indexes for improved search performance (without CONCURRENTLY due to transaction limitations)
CREATE INDEX IF NOT EXISTS idx_profiles_first_name_gin ON public.profiles USING gin(first_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_profiles_last_name_gin ON public.profiles USING gin(last_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_profiles_username_gin ON public.profiles USING gin(username gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_profiles_name_gin ON public.profiles USING gin(name gin_trgm_ops);
