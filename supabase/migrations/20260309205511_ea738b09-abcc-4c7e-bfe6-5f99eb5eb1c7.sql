...
-- Update handle_new_user to store vendor title in signup_metadata
...
BEGIN
  ...
  -- For vendor signups, store title in signup_metadata
  vendor_title := new.raw_user_meta_data ->> 'title';
  IF vendor_title IS NOT NULL AND vendor_title != '' THEN
    UPDATE public.profiles
    SET signup_metadata = COALESCE(signup_metadata, '{}'::jsonb) || jsonb_build_object('title', vendor_title)
    WHERE id = new.id;
  END IF;
...
-- Backfill: copy title from auth.users.raw_user_meta_data into profiles.signup_metadata for existing vendors
UPDATE public.profiles p
SET signup_metadata = COALESCE(p.signup_metadata, '{}'::jsonb) || jsonb_build_object('title', u.raw_user_meta_data->>'title')
FROM auth.users u
WHERE p.id = u.id
  AND u.raw_user_meta_data->>'title' IS NOT NULL
  AND u.raw_user_meta_data->>'title' != ''
  AND (p.signup_metadata IS NULL OR p.signup_metadata->>'title' IS NULL);
