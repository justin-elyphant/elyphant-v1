-- Fix queue_post_purchase_followup to use order_items table instead of non-existent items column
...
BEGIN
  ...
  IF NEW.status = 'delivered' AND (OLD.status IS NULL OR OLD.status != 'delivered') THEN
    ...
    -- Extract product names from order_items table
    SELECT string_agg(oi.product_title, ', ')
    INTO product_names
    FROM order_items oi
    WHERE oi.order_id = NEW.id;
    
    -- If order_items doesn't exist or is empty, try cart_data as fallback
