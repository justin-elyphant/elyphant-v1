...
const RecurringToggle = ({ form, validationError }: RecurringToggleProps) => {
  const isRecurring = form.watch("isRecurring");
  const eventType = form.watch("eventType");

  // Auto-suggest recurring type based on event type
  React.useEffect(() => {
    if (isRecurring && !form.getValues("recurringType")) {
      if (eventType === "birthday" || eventType === "anniversary") {
        form.setValue("recurringType", "yearly");
      } else {
        form.setValue("recurringType", "yearly");
      }
    }
  }, [isRecurring, eventType, form]);
