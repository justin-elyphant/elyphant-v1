-- =====================================================
-- CRITICAL SECURITY FIXES FOR DATA EXPOSURE ISSUES
-- =====================================================
...
-- 6. ENSURE MISSING ADMIN AUTHORIZATION FUNCTION EXISTS
-- Create helper function to check if user has admin-level business permissions
...
BEGIN
  ...
  RETURN EXISTS (
    ...
    AND (
      (action_type = 'view' AND ba.can_view_payment_methods = true) OR
      (action_type = 'manage' AND ba.can_manage_payment_methods = true)
    )
  );
END;
$$;