    ...
    <div className="mb-8 md:mb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
        {UNIVERSAL_CATEGORIES.map((category) => (
          <div
            key={category.id}
            className="group relative p-3 md:p-4 bg-background rounded-lg shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer border border-border h-full touch-target-48 touch-manipulation flex flex-col"
            onClick={() => handleCategoryClick(category.value, category.searchTerm)}
          >
            <div className="text-center flex-1 flex flex-col">
              <div className="text-muted-foreground mb-2 md:mb-3 group-hover:scale-110 transition-transform duration-300 flex justify-center">
                {category.icon && <category.icon className="h-5 w-5 md:h-6 md:w-6" />}
              </div>
              <h3 className="text-xs md:text-sm font-semibold text-foreground mb-1 md:mb-2 line-clamp-1">
                {category.name}
              </h3>
              <p className="text-xs text-muted-foreground mb-2 line-clamp-2 leading-relaxed flex-1">
                {category.description}
              </p>
              <Button
                variant="ghost"
                size="sm"
                className="w-full group-hover:bg-muted/50 text-xs border border-border hover:border-border h-7 md:h-8 mt-auto"
              >
                Browse
