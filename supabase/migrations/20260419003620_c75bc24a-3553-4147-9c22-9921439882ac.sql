-- 1) Issue Larry's $100 welcome credit (referred user gets $100 to spend)
INSERT INTO public.beta_credits (user_id, amount, type, description, referral_id)
SELECT 'ddb40385-5a22-44af-b782-356e1954de73', 100, 'welcome',
       'Welcome credit — joined via beta invite', 'a911554d-c1bb-44ee-960f-b3c2e988db33'
WHERE NOT EXISTS (
  SELECT 1 FROM public.beta_credits
  WHERE user_id = 'ddb40385-5a22-44af-b782-356e1954de73'
    AND type = 'welcome'
);

-- 2) Update analytics RPC to include both 'issued' (referrer rewards) and 'welcome' (referred user credits)
...
BEGIN
  WITH beta_testers AS (
    ...
    WHERE bc.type IN ('issued', 'welcome')
  ),
  funnel AS (
    SELECT
  ...
  engagement AS (
    SELECT
      ...
      COALESCE(
        ...
        NULLIF((SELECT sum(amount) FROM public.beta_credits WHERE type IN ('issued', 'welcome')), 0),
        0
