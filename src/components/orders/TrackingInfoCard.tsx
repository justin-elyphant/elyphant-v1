...
import { Copy, ExternalLink, Truck, CheckCircle2, Circle, Camera } from "lucide-react";
...
const TrackingInfoCard = ({ order }: TrackingInfoCardProps) => {
  ...
  const hasTrackingInfo =
    displayTrackingNumber || order.status === "shipped" || order.status === "delivered";
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <span className="text-sm font-normal text-muted-foreground">{carrierName}</span>
        </CardTitle>
      </CardHeader>
          ...
          <div className="space-y-3">
            <div className="p-3 bg-muted/50 rounded-lg">
              ...
              {steps.find((s) => s.id === "delivered")?.timestamp && (
                <p className="text-xs text-muted-foreground mt-0.5">
                  {format(
                    new Date(steps.find((s) => s.id === "delivered")!.timestamp!),
        ...
        {displayTrackingNumber && (
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <code
                className="flex-1 px-2.5 py-1.5 bg-muted rounded text-xs font-mono"
