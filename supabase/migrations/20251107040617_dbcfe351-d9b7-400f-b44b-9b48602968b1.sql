-- Create function to track wishlist purchases and queue notification email
...
-- Create trigger on wishlist_item_purchases
DROP TRIGGER IF EXISTS trigger_wishlist_purchase_notification ON wishlist_item_purchases;
CREATE TRIGGER trigger_wishlist_purchase_notification
  AFTER INSERT ON wishlist_item_purchases
  FOR EACH ROW
  EXECUTE FUNCTION track_wishlist_purchase_and_notify();

-- Add comment
COMMENT ON FUNCTION track_wishlist_purchase_and_notify() IS 'Tracks wishlist item purchases and queues notification email to wishlist owner';