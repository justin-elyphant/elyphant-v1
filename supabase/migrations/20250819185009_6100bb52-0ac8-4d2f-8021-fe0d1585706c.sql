        ...
        LOOP
            ...
            IF wishlist_record->'items' IS NOT NULL AND jsonb_array_length(wishlist_record->'items') > 0 THEN
                ...
                SELECT 
                    gen_random_uuid(),
                    new_wishlist_id,
                    -- Handle product_id which might be string or integer
                    CASE 
                        WHEN (item->>'product_id') ~ '^[0-9]+$' THEN (item->>'product_id')::integer
                        ELSE NULL -- Skip non-numeric product IDs for now
                    END,
