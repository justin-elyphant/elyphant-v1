...
const DataVerificationSection: React.FC<DataVerificationSectionProps> = ({ friend, onVerificationRequest }) => {
  ...
  return (
    <>
      ...
      {hasUnverifiedData && (
        <Alert variant="destructive" className="mb-2 p-3 text-xs">
          <AlertCircle className="h-3 w-3 text-white" />
          <AlertTitle className="text-xs text-white mb-1">Auto-gifting unavailable</AlertTitle>
          <AlertDescription className="text-xs text-red-200 leading-tight">
            {hasBlockedData 
