-- Enhanced security for business payment methods
...
BEGIN
  ...
  CASE TG_OP
    ...
    WHEN 'DELETE' THEN
      ...
      RETURN OLD;
    WHEN 'SELECT' THEN
      -- Note: SELECT triggers don't fire in PostgreSQL, this is just for completeness
      RETURN NULL;
  END CASE;
...
CREATE POLICY "Authorized manage access only"
  ON public.business_payment_methods
