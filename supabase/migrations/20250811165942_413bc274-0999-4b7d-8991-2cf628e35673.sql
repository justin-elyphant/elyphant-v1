-- Security Fix Phase 2: Database Hardening and Remaining Issues
...
-- Fix 3: Update all functions to use secure search_path
-- This prevents search path manipulation attacks

-- Update existing functions to use SECURITY DEFINER SET search_path = ''
-- First, let's update the functions that are missing this protection
...
-- Create trigger for pricing_settings audit
...
-- Add audit trigger for message_rate_limits access
CREATE OR REPLACE FUNCTION public.audit_rate_limits_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $function$
BEGIN
  -- Audit logging for rate limits access
  INSERT INTO public.admin_audit_log (
    admin_user_id,
    action_type,
    target_type,
...
-- Create trigger for message_rate_limits audit
DROP TRIGGER IF EXISTS audit_rate_limits_trigger ON public.message_rate_limits;
CREATE TRIGGER audit_rate_limits_trigger
  AFTER SELECT OR UPDATE OR DELETE ON public.message_rate_limits
  FOR EACH ROW EXECUTE FUNCTION public.audit_rate_limits_access();